...
const getAllFilesIn = function (dir, filelist) {
  ...
  files.forEach(function (file) {
    if (fs.statSync(path.join(dir, file)).isDirectory()) {
      filelist = getAllFilesIn(path.join(dir, file), filelist);
    } else {
...
const cucumberOptsRequire = getAllFilesIn('./src/step-definitions/').concat([
...
exports.config = {
  ...
  reporters: ['cucumber', 'junit', 'json', 'allure'],
  reporterOptions: {
    json: {
      outputDir: 'output/json',
    },
    junit: {
      outputDir: 'output/junit-results',
    },
    allure: {
  ...
  // Additional timeouts. See http://webdriver.io/guide/testrunner/timeouts.html for more details
  timeouts: {
    script: 3000,
    implicit: 3000,
  },
  ...
  before() {
    browser.windowHandleSize({
      width: browser.options.windowHandleSize.width,
      height: browser.options.windowHandleSize.height,
    });
  },
